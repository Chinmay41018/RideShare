...
if __name__ == "__main__":
    ...
    rider_write = deepcopy(body)

    rider = {'from': body['from']['S'], 
          'to': body['to']['S'],
          'start_date': body['start_date']['S'],
          'start_time': body['start_time']['S'], 
          'num_riders': body['num_riders']['N'] ,
          'rider_id' : body['rider_id']['S'],
          'city': body['city']['S'],
          'matched' : body['matched']['S'],
          'username' : body['username']['S']
         }

    rider_info_string = json.dumps(rider)
                ...
                for j in range(i+1, len(waypoints_tmp)+1):
                    ...
                    for k in range(len(waypoints_new)):
                        ...
                        if  time_rchd > r_start - timedelta(minutes=time_epsilon) and time_rchd < r_start + timedelta(minutes=time_epsilon):
                            ...
                            time_criteria_matched = False
                            break
                    print(time_criteria_matched, distance_criteria_matched, trip['trip_id'], trip_distance, original_distance, distance_epsilon, time_rchd, r_start)
                    if time_criteria_matched and distance_criteria_matched:
                        this_distance_epsilon = trip_distance/original_distance
                        if this_distance_epsilon < min_distance_epsilon:
                            min_distance_epsilon = this_distance_epsilon
    ...
    if add_to_queue:
        ...
        print('Hello Hello, Did I send sms??')
        print(lex_response('ride not found, you have been placed in the queue'))
    print('Done!')
